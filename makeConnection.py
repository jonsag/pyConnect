            ...
            if userNo >= 0:
                ...
                if verbose:
                    print(
                        ...
                        + connection["username"]
                        + " who has user index "
                        + str(connection["userNo"])
                    )
                    if show:
                        print(
                            "    Use password '"
                            + decryptPassword(f_key, connection["cryptPasswd"], verbose)
                            + "'"
                        )
                outputList, errorList = paramikoRunCmd(
                    f_key,
                    connection["ip"],
                    connection["port"],
                    connection["username"],
                    connection["cryptPasswd"],
                    cmd,
                    verbose,
                )
                print(
...
def selectConnection(f_key, connectionFile, show, verbose):
    ...
    for section in sections:
        ...
        connectionList.append(
            {u"number": connectionNo, u"ip": section, u"host": hostname, u"port": port}
        )
...
def pxsshConnect(f_key, ip, port, username, cryptPasswd, verbose):
    ...
    try:
        s = pxssh.pxssh()
        s.login(
            ip, username, decryptPassword(f_key, cryptPasswd, verbose), port=int(port)
        )
